...
export default function GallerySection() {
  ...
  galleries[0].images = allImages;

  const active = galleries.find((g) => g.key === activeKey)!;
  const images = active.images ?? allImages;
  const featured = images.slice(0, 6);
  const strip = images.slice(0, Math.min(images.length, 10));

  const next = () => setLightboxIndex((p) => (p !== null ? (p + 1) % images.length : null));
  const prev = () => setLightboxIndex((p) => (p !== null ? (p - 1 + images.length) % images.length : null));
  ...
  return (
    <section id="gallery" className="relative overflow-hidden bg-transparent py-24 md:py-32 font-sans">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6">
        ...
        <div className="mb-12 flex flex-col items-center text-center">
          ...
          <p className="mx-auto mt-6 max-w-xl text-base italic text-gray-400">
        ...
        <div className="mb-8 flex flex-wrap justify-center gap-2">
          {galleries.map((g) => (
            <button
              ...
              className={`relative flex items-center gap-2.5 rounded-full border px-6 py-2.5 text-[10px] font-bold uppercase tracking-[0.28em] transition-all duration-400 ${
                activeKey === g.key
            ...
            >
              ...
              <span className={`text-[8px] tabular-nums ${activeKey === g.key ? 'text-gold-400/60' : 'text-gray-600'}`}>
                {g.key === 'all' ? allImages.length : (g.images?.length ?? 0)}
              </span>
            </button>
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div>
              <h3 className="font-display text-3xl font-bold uppercase tracking-tight text-white">{active.title}</h3>
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="grid auto-rows-[11rem] grid-cols-2 gap-4 md:auto-rows-[13rem] md:grid-cols-12 md:gap-5"
          >
            {featured.map((img, i) => {
        ...
        {/* ── Filmstrip ── */}
        <div className="mt-5 rounded-[1.8rem] border border-white/8 bg-black/15 p-4 shadow-premium backdrop-blur-xl">
          <div className="no-scrollbar flex gap-3 overflow-x-auto pb-1">
            {strip.map((img, i) => (
              <button
                ...
                type="button"
                onClick={() => setLightboxIndex(i)}
                className="group relative h-20 w-28 shrink-0 overflow-hidden rounded-[1.2rem] border border-white/8 transition-all hover:border-gold-400/30 md:h-24 md:w-36"
              >
      ...
      <AnimatePresence>
        {lightboxIndex !== null && lightboxImg && (
          ...
          >
            {/* Top bar */}
            <div className="absolute inset-x-0 top-0 z-[110] flex items-center justify-between px-6 py-5 md:px-10">
              <div className="rounded-[1.5rem] border border-white/10 bg-black/30 px-5 py-3 backdrop-blur-xl">
                ...
                <p className="mt-1 font-display text-lg font-bold uppercase tracking-tight text-white">{lightboxImg.alt}</p>
              ...
              <button
                ...
                className="flex h-12 w-12 items-center justify-center rounded-full border border-white/10 bg-black/30 text-white/70 backdrop-blur-xl transition-all hover:border-gold-400/40 hover:text-gold-200"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <motion.div
              ...
              className="relative z-[105] mx-auto flex h-full max-w-6xl items-center justify-center px-6 pb-36 pt-28 md:px-20"
            ...
            {/* Bottom strip */}
            <div className="absolute inset-x-0 bottom-0 z-[110] px-6 pb-5 md:px-10">
              <div className="mx-auto max-w-5xl rounded-[1.8rem] border border-white/10 bg-black/30 p-4 backdrop-blur-xl">
                <div className="no-scrollbar flex gap-3 overflow-x-auto">
                  {strip.map((img, i) => (
                    <button
                      ...
                      className={`relative h-16 w-24 shrink-0 overflow-hidden rounded-xl border transition-all md:h-20 md:w-28 ${
